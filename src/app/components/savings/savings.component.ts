...
export class SavingsComponent implements OnInit {
  ...
  fetchAllBudgetList(month: any, year: any) {
    ...
    this.budgetService.getCurrentBudget(month, year).subscribe((data: any) => {
      ...
      for (let bud of data) {
        if (bud.budgetAmount != null && bud.budgetAmount !== '') {
          this.total = this.total + Number(bud.budgetAmount);
        }
      }
  ...
  groupDataByMain(data: any[]): any {
    const grouped = {};
    data.forEach((item) => {
      const mainCategory = item.categoryApiDTO.mainCategory;
      const subCategory = item.categoryApiDTO.subCategory;

      let cat = {
        category: item.categoryApiDTO.category,
        mainCategory: mainCategory,
        subCategory: subCategory,
        budgetAmount: item.budgetAmount,
        date: item.date,
        categoryId: item.categoryApiDTO.categoryId,
        categoryGroup: item.categoryApiDTO.categoryGroup,
  ...
  toggleAllExpanded() {
    ...
    this.setAllExpandedState(this.isAllExpanded);
  }
  ...
  mainTotal(main: string): number {
    ...
    Object.values(subs).forEach((arr: any) => {
      if (Array.isArray(arr)) {
        arr.forEach(item => {
          total += Number(item.budgetAmount || 0);
        });
      }
    });
    return total;
